...
def chromium_page_with_state(request: SubRequest, initialize_browser_state, playwright: Playwright) -> Page:
    ...
    # browser.close()
    #
    # allure.attach.file(f'./tracing/{request.node.name}.zip', name='trace',extension='zip') # файл трейсинга
    # allure.attach.file(page.video.path(), name='video', attachment_type=AttachmentType.WEBM) # Видео